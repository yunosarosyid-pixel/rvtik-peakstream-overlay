...
export default function App() {
  ...
  const [connStatus, setConnStatus] = useState<'local' | 'supabase'>('local');
  ...
  useEffect(() => {
    ...
    if (sync) {
      ...
      sync.onConnectionChange((status: { mode: 'local' | 'supabase' }) => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border border-emerald-500/30 bg-emerald-500/10 text-emerald-400">
              ...
              <span>{connStatus === 'supabase' ? 'Supabase Realtime' : 'Live Sync Active'}</span>
            </div>
